...
WebLFP 是一个本地优先的 LFP 推理 Web 应用。它仅使用 LFP 输入生成 128 维
CLIP 对齐表征，并提供原项目一致的窄波/非窄波 `presence` 与 `count` 窗口级解码。
...
- 按锁定配置重采样、切窗并生成 CLIP 对齐 LFP 隐空间。
...
执行 `git lfs pull`。两份锁定权重为：

| 用途 | 文件 | SHA-256 |
| --- | --- | --- |
| CLIP 对齐 LFP 编码器 | `models/clip-lfp-best/model.pt` | `5d197f3e96ed274f34f55293b5cb993bcc3edb8d0d5d8eb3d09f69983c025e2d` |
| 窄波/非窄波解码器 | `models/spike-type-decoder/model.pt` | `0a68f7ce165eb52aaffda759146ea8be438de6b010aee731e19a36bbe8305809` |
...
该表示和参考解码器仅在报告所述 KA 慢性癫痫小鼠海马数据与任务上得到验证。
